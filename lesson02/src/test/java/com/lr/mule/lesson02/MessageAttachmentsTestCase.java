...
public class MessageAttachmentsTestCase extends FunctionalTestCase{
	...
	protected String getConfigResources() {
		...
		return "./src/test/resources/globleconfig.xml,./src/test/resources/08_message-attachments.xml";
	}
	@Test
	...
	public void attachPdfUsageReport() throws Exception {
        ...
        MuleMessage result = muleClient.send("vm://add-attachments.in",TEST_PAYLOAD,Collections.<String,Object>singletonMap("pdfFilePath",usageReportPdfFilePath));

        assertThat(result.getInboundAttachmentNames().contains(TEST_USAGE_REPORT_PDF),is(true));
	   ...
	   @Test
	    public void processAttachments() throws Exception
	    {
	        MuleMessage message = new DefaultMuleMessage("fake email content",
	            muleContext);
	        File orderFile = new File(getPdfResourceUrl(TEST_ORDER_PDF).toURI());
	        message.addOutboundAttachment("1-" + orderFile.getName(),orderFile, "application/pdf");
//	        message.addOutboundAttachment("2-" + orderFile.getName(),orderFile, "application/pdf");
	        muleContext.getClient().dispatch("vm://email.orders.in",message);
	        ...
	        assertThat(ftc.getReceivedMessage(1) instanceof javax.activation.DataHandler,is(true));
	        
	        assertThat(ftc.getReceivedMessage(2) instanceof javax.activation.DataHandler,is(true));
	    }
